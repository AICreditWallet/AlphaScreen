  ...
  const PricingView = () => (
    <div className="min-h-screen pt-48 px-12 pb-20 space-y-24 cinema-bg relative text-center">
      ...
      <div className="relative z-10 text-center space-y-10 max-w-5xl mx-auto">
        <h2 className="text-5xl md:text-7xl font-black uppercase tracking-tighter text-white leading-none">Choose Your <span className="text-accent italic">Scale.</span></h2>
        <p className="text-white/40 text-xl font-medium tracking-tight leading-relaxed max-w-3xl mx-auto">Select your production tier to enter the autonomous environment.</p>
      </div>
        ...
        {[
          ...
          <motion.div key={tier.title} whileHover={{ y: -10 }} className={`glass-panel p-10 md:p-20 rounded-[3rem] md:rounded-[4rem] flex flex-col justify-between border-white/20 hover:border-white/40 transition-all duration-700 shadow-2xl ${tier.best ? 'border-accent/40 bg-accent/[0.03]' : ''}`}>
            <div className="space-y-10 text-left">
              <h3 className="text-4xl md:text-6xl font-black uppercase italic tracking-tighter text-white">{tier.title}</h3>
              <p className="text-white/40 text-base md:text-xl leading-relaxed font-medium">{tier.desc}</p>
            ...
            <div className="pt-12 md:pt-16 border-t border-white/10 text-left">
              <span className="text-4xl md:text-7xl font-black text-white mb-8 md:mb-10 block">{tier.price}</span>
              <button onClick={() => { setProjectType(tier.type as any); setView("landing"); }} className={`w-full ${tier.best ? 'bg-accent text-white' : 'bg-white text-black hover:bg-accent hover:text-white'} py-5 md:py-8 rounded-full font-black uppercase tracking-[0.3em] transition-all transform active:scale-95 shadow-2xl text-[10px] md:text-sm`}>
    ...
    <div className="min-h-screen pt-40 px-8 pb-32">
      <div className="max-w-7xl mx-auto flex items-center justify-between mb-20 px-8 text-left">
        <div className="space-y-4 text-left">
          ...
          <h2 className="text-4xl md:text-7xl font-black tracking-tighter uppercase italic text-white leading-none">Studio <span className="text-accent italic text-gradient">Environment.</span></h2>
        </div>
  ...
  return (
    <main className="min-h-screen bg-black text-white selection:bg-accent overflow-x-hidden">
      ...
      <footer className="py-20 md:py-24 px-8 md:px-20 flex flex-col md:flex-row items-center justify-between gap-12 md:gap-20 border-t border-white/5 bg-black z-50 relative text-left">
        ...
        <p className="text-[9px] md:text-[10px] font-bold text-white/10 uppercase tracking-[0.3em]">© 2026 AlphaScreen. Disrupting Cinema.</p>
      </footer>
    </main>
  );
}
